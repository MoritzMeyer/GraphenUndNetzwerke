...
namespace GraphCollection.SearchAlgorithms
{
    public static class DijkstraExtension
    {
        #region Dijkstra
        /// <summary>
        /// Ermittelt die kürzeseten Wege zwischen alle Knoten im Graphen
        /// </summary>
        ...
        public static Graph<T> Dijkstra<T>(this Graph<T> graph, Vertex<T> start)
        {
            if (!graph.IsWeighted)
            {
        ...
        #region Dijkstra
        public static Graph<T> Dijkstra<T>(this Graph<T> graph, Vertex<T> start, Vertex<T> goal)
        {
            if (!graph.IsWeighted)
            {
                throw new ArgumentException("Der Dijkstra Algorithmus kann nur auf Graphen mit gewichteten Kanten durchgeführt werden.");
